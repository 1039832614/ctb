...
class Complain extends Shop
{
	...
	/**
	 * 获取运营商的名称，投诉页面获取运营上名称，
     * 新版本使用  
	 */
	public function agent()
	{
		$aid = $this->getAgent();
		$agent_name = Db::table('ca_agent')
                        ->where('aid',$aid)
                        ->value('company');
		if($agent_name){
    ...
    public function complaint()
    {
        ...
        if($validate->check($data)) {
            ...
            if ($data['type'] == 2 ){
                ...
                // 查询维修厂市级ID
                $city_id = Db::table('co_china_data')
                            ->where('name',$shop_info['city'])
                            ->value('id');
                // 查询 省级ID
                $pro_id = Db::table('co_china_data')
                            ->where('name',$shop_info['province'])
                            ->value('id');
                // 查询维修厂负责人姓名
                $name = Db::table('cs_shop')
                        ->where('id',$this->sid)
                        ->field('company,leader')
                        ->find();
                $arr = [
                    'sm_id'        => $data['sm_id'],            //服务经理id
                    'uid'          => $this->sid,                //当前用户id
                    'pro_id'       => $pro_id,                   //省份id
                    'city_id'      => $city_id,                  //城市id
                    'county_id'    => $shop_info['county_id'],   //区县id
                    'name'         => $name['leader'],           //姓名
                    'company'      => $name['company'],          //公司名称
                    'phone'        => $shop_info['serphone'],    //维修厂联系方式
                    'title'        => $data['title'],            //标题
                    'content'      => $data['content'],
     ...
     * @return [type] [description]
     */
    public function getUnWithdrawList()
    {
    ...
    public function detail()
    {
        ...
        $par = input('post.');
        if ($par['type'] == 1){
            $data = Db::table('cs_complain')
                    ->field('title,content,create_time')
                    ->where('id',$par['id'])
                    ->find();
        }elseif ($par['type'] == 2){
            $data = Db::table('sm_complaint')
                    ->field('title,content,create_time')
                    ->where('id',$par['id'])
                    ->find();
        }
